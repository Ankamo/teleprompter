...
export default function TeleprompterProStudio() {
  ...
  useEffect(() => {
    ...
    peer.on("open", (id) => {
      setPeerId(id);
      setConnectionStatus(connectionMode === "host" ? "Esperando cámara remota..." : "Listo para conectar");
    });

    peer.on("call", async (call) => {
      // Host recibe la llamada de la cámara
      try {
  ...
  // Conectar como cámara (dispositivo secundario)
  ...
  const startCamera = useCallback(async () => {
    if (connectionMode === "host" && isConnected) return; // ya tiene stream remoto
  ...
  useEffect(() => {
    if (connectionMode === "local" || connectionMode === "camera") {
      startCamera();
    }
  ...
  useEffect(() => {
    ...
    recognition.onresult = () => {
      // Cada vez que detecta voz, acelera ligeramente el scroll
      if (isPrompting) {
      ...
      <div className="max-w-6xl mx-auto flex flex-col gap-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-white flex items-center gap-2">
                ...
                <span className="text-[10px] bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded-full">v3.0 Multi-Device</span>
              </h1>
              <p className="text-xs text-zinc-400">PWA • Multi-dispositivo • Voice Tracking • Highlight Band</p>
            </div>
          </div>
            ...
            {connectionMode !== "local" && (
              <>
                <div className="text-xs bg-zinc-950 px-3 py-2 rounded-xl border border-zinc-700">
                  Tu ID: <span className="font-mono text-emerald-400">{peerId || "generando..."}</span>
                </div>
                {connectionMode === "camera" && (
                  <>
                    ...
                    />
                    <button onClick={connectAsCamera} className="px-4 py-2 bg-emerald-600 rounded-xl text-xs font-semibold">
                      Conectar
                    </button>
                  </>
                )}
          ...
          <div className="lg:col-span-2">
            <div className={`bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden ${aspectRatio === "9:16" ? "max-w-sm mx-auto" : ""}`}>
              ...
              <div className={`relative bg-black ${aspectRatio === "16:9" ? "aspect-video" : aspectRatio === "9:16" ? "aspect-[9/16]" : "aspect-square"}`}>
                ...
                {overlayImage && (
                  <img src={overlayImage} className={`absolute object-contain pointer-events-none ${imagePosition.includes("top") ? "top-4" : "bottom-4"} ${imagePosition.includes("left") ? "left-4" : "right-4"}`} style={{ width: imageSize, height: imageSize }} />
                )}
                  ...
                  <div className="absolute inset-0 bg-black/50 z-30 flex flex-col">
                    ...
                    {showHighlightBand && (
                      <div
                        ...
                        style={{ top: `calc(${eyeLinePos}% - 45px)`, height: "90px" }}
                      />
                    )}
              ...
              <div className="p-3 bg-zinc-950 border-t border-zinc-800 flex flex-wrap justify-between gap-2">
                ...
                {!isRecording ? (
                  <button onClick={startRecordingWithCountdown} disabled={!cameraReady} className="px-5 py-2.5 bg-red-600 hover:bg-red-500 rounded-xl text-xs font-bold uppercase disabled:opacity-40">
                    🔴 Grabar
                  </button>
                ) : (
                  <button onClick={stopRecording} className="px-5 py-2.5 bg-white text-black rounded-xl text-xs font-bold uppercase">
                    Detener
                  </button>
                )}
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5">
              ...
              <div className="grid grid-cols-3 gap-2">
                {(["16:9", "9:16", "1:1"] as const).map(r => (
                  <button key={r} onClick={() => setAspectRatio(r)} className={`py-2 text-xs rounded-xl border ${aspectRatio === r ? "bg-emerald-600/20 text-emerald-400 border-emerald-600/40" : "border-zinc-700"}`}>
                    {r}
                  </button>
                ))}
              </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 space-y-3">
            <h3 className="text-xs font-mono text-zinc-400 uppercase">Redes (se graban en el video)</h3>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 space-y-3">
            <h3 className="text-xs font-mono text-zinc-400 uppercase">Logo / Marca de agua</h3>
